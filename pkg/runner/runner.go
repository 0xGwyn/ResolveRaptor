...
func (runner *Runner) Start() error {
	...
	err = mergeFiles(path.Join(runner.options.domain, "generated.subs"), path.Join(runner.options.domain, "subfinder.subs"), path.Join(runner.options.domain, "shuffledns_phase1.in"))
	if err != nil {
		return err
	}

	//merge crt.sh subdomains with abuseipdb subdomains then sort and uniquify them
	err = mergeFiles(path.Join(runner.options.domain, "crtsh.subs"), path.Join(runner.options.domain, "abuseipdb.subs"), path.Join(runner.options.domain, "shuffledns_phase1.in"))
	if err != nil {
		return err
	}

	//run shuffledns
	err = runShuffledns(runner.options.domain, runner.options.resolver, path.Join(runner.options.domain, "shuffledns_phase1.in"), path.Join(runner.options.domain, "shuffledns_phase1.out"))
	if err != nil {
		return err
	}

	//merge the resolved subdomains and the subfinder output for permulation tools
	err = mergeFiles(path.Join(runner.options.domain, "shuffledns_phase1.out"), path.Join(runner.options.domain, "subfinder.subs"), path.Join(runner.options.domain, "permutation.in"))
	if err != nil {
		return err
	}

	//run dnsgen on the resolved subdomains and the subfinder output merged file
	err = runDnsgen(path.Join(runner.options.domain, "permutation.in"), path.Join(runner.options.domain, "shuffledns_phase2.in"), runner.options.fast)
	if err != nil {
		return err
	}
	...
	err = mergeFiles(path.Join(runner.options.domain, "shuffledns_phase1.out"), path.Join(runner.options.domain, "shuffledns_phase2.out"), path.Join(runner.options.domain, runner.options.output))
	if err != nil {
		return err
	}

	//print results to stdout
	printResults(runner.options.silent, path.Join(runner.options.domain, runner.options.output))
